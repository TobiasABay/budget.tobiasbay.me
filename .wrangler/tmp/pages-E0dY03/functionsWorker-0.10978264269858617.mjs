...
var onRequestDelete = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    const deleteResult = await env.budget_db.prepare("DELETE FROM budgets WHERE user_id = ? AND year = ?").bind(userId, year).run();
    ...
    const result = await env.budget_db.prepare("SELECT year FROM budgets WHERE user_id = ? ORDER BY year ASC").bind(userId).all();
    const budgets = result.results.map((row) => row.year);
...
var onRequest = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    if (request.method === "POST") {
      ...
      try {
        await env.budget_db.prepare("INSERT INTO budgets (user_id, year) VALUES (?, ?)").bind(userId, year).run();
        const result = await env.budget_db.prepare("SELECT year FROM budgets WHERE user_id = ? ORDER BY year ASC").bind(userId).all();
        const budgets = result.results.map((row) => row.year);
        return new Response(JSON.stringify({ success: true, budgets }), {
          headers: { ...corsHeaders, "Content-Type": "application/json" }
...
// ../.wrangler/tmp/pages-E0dY03/functionsRoutes-0.7913786540318368.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-0Nupfp/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-0Nupfp/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.10978264269858617.mjs.map
